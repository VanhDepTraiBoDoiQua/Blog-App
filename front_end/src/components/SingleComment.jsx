...
const SingleComment = ({comment, postId}) => {
    ...
    const mutation = useMutation({
        ...
        onError: (res) => {
            toast.error("Something went wrong!" + res.respone.data);
        }
    });
        ...
        <div className="p-4 bg-slate-100 rounded-xl">
            <div className="flex items-center gap-4">
                ...
                <span className="text-sm text-gray-400">{format(comment.createdAt)}</span>
                {
                    ...
                    <span onClick={() => mutation.mutate()} className="flex items-center gap-1 text-xs text-red-300 hover:text-red-500 cursor-pointer ml-auto">
